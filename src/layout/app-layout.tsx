...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['system-owner', 'administrator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<CustomerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/customers` : null),
    () => getCustomers({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Customers',
      path: isTenantUser ? `/customers/view/${data?.[0]?.id}` : '/customers',
      entity: 'customer',
    ...
    { name: 'Topics', path: '/topics', entity: 'topic', service: AccessServiceEnum.PROJECT, icon: FiTag },
