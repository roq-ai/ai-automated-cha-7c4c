...
import { getQueryTopicById } from 'apiSdk/query-topics';
import { QueryTopicInterface } from 'interfaces/query-topic';

function QueryTopicViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('query_topic', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/query-topics/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('query', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Query"
                    text={
                      <Link as={NextLink} href={`/queries/view/${data?.query?.id}`}>
                        {data?.query?.content}
                      </Link>
                    }
                  />
