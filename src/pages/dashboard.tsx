...
import { getResponses } from 'apiSdk/responses';
import { ResponseInterface } from 'interfaces/response';
import { getQueryTopics } from 'apiSdk/query-topics';
import { QueryTopicInterface } from 'interfaces/query-topic';

import { QueryTopicListPage as ShowCaseList } from 'pages/query-topics';
...
function DashboardPage() {
  ...
  const responseStat = useSWR<PaginatedInterface<ResponseInterface>>(
    () => `/responses`,
    () => getResponses({ limit: 1, offset: 0 }),
  );

  const queryTopicStat = useSWR<PaginatedInterface<QueryTopicInterface>>(
    () => `/query-topics`,
    () => getQueryTopics({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Responses',
      result: responseStat?.data?.totalCount,
      isLoading: responseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Query Topics',
      result: queryTopicStat?.data?.totalCount,
      isLoading: queryTopicStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`query-topics`} passHref legacyBehavior>
                  <Button
